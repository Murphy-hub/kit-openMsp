...
func New(ctx context.Context, opts Options) (*Limiter, error) {
	...
	return newRedisLimiter(ctx, &opts)
}
...
func newRedisLimiter(ctx context.Context, opts *Options) (limiter *Limiter, err error) {
	sha1, err := opts.Client.RateScriptLoad(ctx, lua)
	if err != nil {
		//panic(err)
		return nil, err
	}
	...
	return &Limiter{r, opts.Prefix}, nil
}
...
local limit = redis.call('hmget', KEYS[1], 'ct', 'lt', 'dn', 'rt', 'kr')
local rate = 0
if limit[1] then
  ...
  rate = tonumber(res[2] - res[1])
  redis.call('hmset', KEYS[1], 'kr', rate)
  ...
  redis.call('hmset', KEYS[1], 'ct', res[1], 'lt', res[2], 'dn', res[3], 'rt', res[4], 'kr', 0)
