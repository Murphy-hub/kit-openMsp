 ...
 * @LastEditors: gitsrc
 * @LastEditTime: 2020-07-09 11:59:11
 * @FilePath: /ServiceCar/utils/rediscluster/cluster.go
 */
...
func (cluster *Cluster) Do(cmd string, args ...interface{}) (interface{}, error) {
	...
	// todo 这里有两个点待优化，一个是获取槽位，key并不总是args[0].但是这个优先级比较低。
	// 第二个是需要根据block设置客户端超时时间。目前cluster没有做，需要获取到底层的conn，然后设置deadline。
	// 需要注意不要对其他命令造成影响，所以暂时这个优化往后放下。
